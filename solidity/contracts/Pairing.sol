...
library Pairing {
    ...
    struct GTPoint {
        // non_residue : c0 = 9 / c1 = 1
        uint[6] c0; // c0[0] = c0.c0.c0, c0[1] = c0.c0.c1, ..., c0[5] = c0.c2.c1
        uint[6] c1; // c1[0] = c1.c0.c0, ..., c1[5] = c1.c2.c1
    }
    ...
    function curveMul(G1Point memory _point, uint _scalar) internal returns (G1Point memory) {
        ...
        assembly {
            success := call(sub(gas(), 20), 7, 0, input, 0x80, result, 0x60)
        }
     ...
     * @param p1 G1 point to add to the second
     * @param p2 G1 point to add to the first
     * @return r The resulting G1 point.
     */
